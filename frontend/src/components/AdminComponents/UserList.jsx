...
const UserList = () => {
  ...
  useEffect(()=>{
      ...
      const fetchData = async()=>{
         ...
         const response = await axios.get('https://dream-house-deal-1.onrender.com/api/admin-user-list')
   ...
   { data.map((item,index)=>{
    return(
      ...
      <th scope="row">{index+1}</th>
         ...
         <td> {item?.contact}   </td>
        <td>{item?.address}    </td> 
       <td><img   height="60" width="100" src={`https://dream-house-deal-1.onrender.com/img/${item?.profile}`} alt=''/>    </td>
    </tr>
    )
